...
def fetch_addisgpt(limit):
    ...
    for row in _parquet_rows(path):
        ...
        if limit and len(out) >= limit:
            break
    return _write('addisgpt.txt', out[:limit] if limit else out)
...
def write_sft(out_path, limit=0):
    """Emit instruction→answer pairs from AddisGPT + FineTome as SFT jsonl."""
    import pyarrow.parquet as pq
    examples = []
...
def main():
    ...
    if args.sft and any(n in wanted for n in ('addisgpt', 'finetome')):
        print('[sft]')
        try:
